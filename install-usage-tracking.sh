...
chmod +x "$SKILL_DIR/scripts/view_usage.py"
chmod +x "$SKILL_DIR/scripts/usage_server.py"
chmod +x "$SKILL_DIR/scripts/upgrade_data.py"
chmod +x "$SKILL_DIR/scripts/migrate_data.py"
...
echo ""
echo "9. 升级数据..."
if [ -f "$SKILL_DIR/data/usage.jsonl" ] && [ -s "$SKILL_DIR/data/usage.jsonl" ]; then
    python3 "$SKILL_DIR/scripts/upgrade_data.py"
else
    echo "  ✓ 没有历史数据需要升级"
fi
